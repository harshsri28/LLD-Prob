...
1.  **Register Doctor**: A new doctor should be able to register, and mention his/her speciality among (Cardiologist, Dermatologist, Orthopedic, General Physician).
2.  **Declare Availability**: A doctor should be able to declare his/her availability in each slot for the day. For example, the slots will be of 30 mins like 9am-9.30am, 9.30am-10am.
3.  **Search Slots**: Patients should be able to login, and search available slots based on speciality.
4.  **Ranked Slots**: The slots should be displayed in a ranked fashion. Default ranking strategy should be to rank by start time. But we should be able to plugin more strategies like Doctor’s rating etc in future.
5.  **Book Appointment**: Patients should be able to book appointments with a doctor for an available slot.
...
6.  **Cancel Appointment**: Patients can also cancel an appointment, in which case that slot becomes available for someone else to book.
7.  **Waitlist Feature**: If the patient wishes to book a slot for a particular doctor that is already booked, then add this patient to the waitlist. If the patient with whom the appointment is booked originally, cancels the appointment, then the first in the waitlist gets the appointment.
8.  **View Appointments**: A patient/doctor should be able to view his/her booked appointments for the day.
